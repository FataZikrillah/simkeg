...
@section('subtitle', 'Buat alokasi anggaran baru untuk kegiatan')
            ...
            <div class="p-6 border-b border-slate-100 dark:border-slate-800 bg-gradient-to-r from-white to-slate-50/50">
                <div class="flex items-center gap-3">
                    <div class="p-2 bg-maroon-soft/10 rounded-lg">
                        <i class="fas fa-plus text-maroon-soft"></i>
                    </div>
                    <div>
                        <h2 class="text-xl font-bold text-slate-800 dark:text-white">Form Anggaran Baru</h2>
                        <p class="text-slate-500 text-xs mt-1 font-medium">Lengkapi detail di bawah untuk menambah anggaran
                        </p>
                    </div>
                </div>
            </div>

            <form action="{{ route('pimpinan.anggaran.store') }}" method="POST" class="p-6 space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="jumlah" id="jumlah" value="{{ old('jumlah') }}" placeholder="0"
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="keterangan" id="keterangan" rows="3" placeholder="Tambahkan catatan jika perlu..."
                        class="w-full bg-slate-50 dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 rounded-xl px-4 py-3 text-sm font-semibold focus:border-maroon-soft focus:ring-0 transition-all @error('keterangan') border-rose-500 @enderror">{{ old('keterangan') }}</textarea>
                    @error('keterangan')
                        <p class="text-[10px] text-rose-500 font-bold uppercase">{{ $message }}</p>
                    @enderror
                </div>
