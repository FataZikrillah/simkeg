    ...
    <tr class="hover:bg-slate-50/50 transition-colors group">
        <td class="px-6 py-4">
            <div class="flex flex-col">
                <span class="text-sm font-bold text-slate-700 block truncate max-w-xs">{{ $item->judul }}</span>
        ...
        <td class="px-6 py-4">
            <div class="flex items-center gap-2">
                ...
                <span class="text-xs text-slate-600 font-semibold">{{ $item->user->name ?? 'Unknown' }}</span>
            </div>
        </td>
        <td class="px-6 py-4">
            <span
        ...
        <td class="px-6 py-4">
            <div class="flex items-center justify-end gap-2">
                @if ($item->status == 'pending')
                    <form action="{{ route('pimpinan.laporan.approve', $item->id) }}" method="POST" class="inline">
                        @csrf
                        @method('PATCH')
                ...
                <form action="{{ route('pimpinan.laporan.destroy', $item->id) }}" method="POST" class="inline"
...
<!-- Meta Data for AJAX -->
<tr class="hidden" id="ajax-meta" data-total="{{ $laporans->total() }}"></tr>

@if ($laporans->hasPages())
    <tr class="hidden pagination-wrapper">
        <td colspan="5">
            <div id="ajax-pagination-links">
                {{ $laporans->links() }}
            </div>
        </td>
    </tr>
